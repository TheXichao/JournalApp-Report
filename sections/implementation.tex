...
\begin{table}[H]
\centering
\caption{Group A Skills Found in the Project (which include all the Models and most of Algorithms listed)}
...
\hline
Files(s) organised for direct access &  File Structure shown above\\
\hline
Complex scientific/mathematical/robotics/control/business model & User session persistance as an example complex control model \\
\hline
...
In the future the sorting modules can be used with any key by simply zipping the array with a different key and passing it to the timSort function. 
...
\end{minted}
Before the button is clicked I have made use of the useState hook to store the user's input, and as they user types in the input field, the state is updated.
...
To enable persistent login session in my React app, browser's local storage as well as context is used. Context is used so that the user's information can be stored an accessed throughout the application, avoiding the need of prop drilling\footnotetext{In react information is passed through components from parent to child, Prop Drilling is when a certain pieces of information is passed through many components. This is bad practice as it clogs up the code making it difficult to maintain.}. 
